...
static int load_certs(byte* cert, word32* certSz, byte* key, word32* keySz, const char* certFile, const char* keyFile)
{
    FILE* file;
...
int main(int argc, char** argv)
{
    ...
    certSz = sizeof(cert);
    keySz = sizeof(key);
    ret = load_certs(cert, &certSz, key, &keySz, certFile, keyFile);
    if (ret != 0)
        return -1;
